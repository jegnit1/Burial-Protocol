# Codex 작업 지시 프롬프트: 코드 기반 비주얼을 에셋 기반 제작 구조로 전환
...
이번 작업은 기능 추가 작업이 아니라 개발 방식 전환 작업이다.
...
따라서 기존의 “코드로 도형을 그려서 임시 표현하는 구조”를 점진적으로 “에셋 기반으로 교체 가능한 구조”로 리팩토링해야 한다.

또한 앞으로 Codex가 아트/에셋이 필요한 작업을 코드만으로 억지 구현하지 않도록 프로젝트 지침 문서를 정비해야 한다.

---

# 현재 문제 인식

현재 코드에는 아래와 같은 코드 기반 비주얼 구조가 존재한다.
...
- `Player.gd`
  - 캐릭터 본체는 `AnimatedSprite2D` 기반이지만, 공격 범위/채굴 범위/시각 피드백 일부는 `_draw()` 기반
  - 공격모듈 비주얼은 장착 데이터 기반으로 런타임에 생성됨

이 구조는 기능 검증에는 유리했지만, 앞으로 아트 품질 개선, 에디터 기반 배치, 애니메이션 튜닝, UI 개선 작업에는 한계가 크다.

---

# Codex 작업 지시

## 1. 기존 코드 기반 비주얼 구조를 조사하라

먼저 현재 프로젝트에서 아래 유형을 전부 검색하고 분류하라.

- `_draw()`를 사용해 게임 오브젝트를 직접 그리는 코드
- `draw_rect`, `draw_line`, `draw_circle`, `draw_colored_polygon` 사용 위치
- `Sprite2D`, `AnimatedSprite2D`, `TextureRect`, `NinePatchRect`, `TileMapLayer` 등 에셋 기반 노드가 없는 시각 요소
- 런타임에만 생성되어 Godot 2D 에디터에서 미리 보기 어려운 시각 요소
- UI 요소 중 코드로 좌표/색상/크기를 직접 구성하는 부분

조사 결과를 문서화하고, 각 항목을 아래 기준으로 분류하라.
...
## 2. 에셋 기반 구조로 점진 리팩토링하라

기존 기능을 깨지 말고, 아래 방향으로 리팩토링하라.

---

### 2-1. 무기 / 공격모듈 비주얼

현재 `AttackModuleVisual.gd`의 `_draw_sword()`, `_draw_dagger()`, `_draw_lance()`, `_draw_axe()`, `_draw_greatsword()`, `_draw_drone()` 방식은 임시 도형 표현으로 간주한다.

이를 아래 구조로 전환할 수 있도록 리팩토링하라.
...
각 무기 시각 씬은 가능하면 다음 노드 구조를 갖도록 한다.
...
- 실제 스프라이트 에셋이 없는 경우 임시 placeholder Sprite2D를 만들지 말고, 필요한 에셋 목록을 문서에 남겨라.
- 기존 `_draw()` 도형은 fallback 또는 debug preview로만 남긴다.
- 새 구조는 추후 에셋을 넣으면 Godot 에디터에서 바로 확인 가능해야 한다.
- 무기별 발사 위치, 회전축, 피벗 위치는 `Marker2D`로 조정 가능하게 한다.
...
이를 즉시 전면 교체하지 말고, 다음 두 단계를 구분하라.
...
- 손상된 벽돌 표현은 코드 색상 보간만으로 고정하지 말고, 추후 damage stage sprite를 적용할 수 있게 구조를 분리한다.
...
현재 HUD/UI 구조를 조사하고, 코드 중심 배치 또는 임시 스타일이 있다면 에셋 기반 UI로 전환 가능한 구조를 제안하라.
...
- 패널 배경은 추후 `NinePatchRect` 또는 `TextureRect`로 교체 가능하게 한다.
- 버튼, 카드, 슬롯, 게이지 등 반복 요소는 재사용 씬으로 분리한다.
- 아이콘 에셋이 필요한 경우 필요한 크기와 형식을 문서화한다.

---

## 3. “에셋 필요 여부 판단 규칙”을 문서화하라

`docs/` 아래에 Codex/Agent 작업 지침 문서를 개선하라.

기존에 `AGENTS.md`, `Agent.md`, `agent.md`, `docs/AGENTS.md`, `docs/Agent.md` 등 유사 파일이 있는지 먼저 확인하라.

- 이미 존재하면 해당 파일을 개선하라.
- 없으면 `docs/AGENTS.md`를 새로 생성하라.
- 프로젝트 루트에 Codex가 자동으로 읽는 규칙 파일이 따로 있다면, 그 파일에서도 `docs/AGENTS.md`를 참조하도록 정리하라.

문서에는 반드시 아래 내용을 포함하라.
...
## 4. Godot 에디터 절차 안내 규칙을 추가하라

Codex는 앞으로 코드 수정만 제안하지 말고, Godot 에디터에서 사용자가 직접 해야 하는 작업이 있다면 반드시 절차를 제공해야 한다.
...
- 에셋 우선 판단 규칙
- Godot 에디터 절차 안내 규칙
- 코드 기반 임시 비주얼 금지/제한 규칙
- 에셋 누락 시 사용자에게 요청하는 응답 템플릿
- 작업 완료 보고 형식
...
이번 작업에서 실제 코드 리팩토링은 너무 크게 하지 말고, 먼저 아래까지 수행하라.

1. 현재 코드 기반 비주얼 사용처 조사
2. `docs/AGENTS.md` 생성 또는 개선
3. `docs/visual_refactor_plan.md` 생성
4. `docs/asset_pipeline.md` 생성
5. `docs/godot_editor_workflow.md` 생성
6. 무기 비주얼 1종만 샘플로 에셋 기반 구조를 받을 수 있게 최소 리팩토링
7. 기존 기능이 깨지지 않도록 fallback 유지
8. 변경 내용과 다음 단계 안내 작성
...
이번 작업의 핵심은 “Codex가 앞으로 아트/에셋이 필요한 작업을 코드만으로 억지 해결하지 않게 만드는 것”이다.

기능 구현 중심의 바이브 코딩 방식에서 벗어나, 앞으로는 다음 흐름을 기본으로 삼아라.
...
코드로 임시 도형을 그려서 “완성된 것처럼” 처리하지 마라.

특히 무기, 투사체, VFX, UI, 벽돌, 블록 외형은 최종적으로 에셋 기반 구조가 되어야 한다.
